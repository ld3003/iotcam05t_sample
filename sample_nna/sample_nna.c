...
static void *NNA_Proc(void* pThreadData)
{
    ...
    while (iFrameIdx < pNNAParams->iFrameNum)
    {
        ...
        if (SUCCESS == QG_MPI_VI_GetFrame(pVirViParam->iViDev, pVirViParam->iViChn, &stFrameInfo, 500))
        {
            // 获取VirVi数据成功
            ...
            if(box_num == 0)
            {
                ...
                // 释放YUV数据
                QG_MPI_VI_ReleaseFrame(pVirViParam->iViDev, pVirViParam->iViChn, &stFrameInfo);
            }
            else
            {
                pthread_mutex_lock(&g_stResult_lock);
...
static void *VENC_Proc(void* pThreadData)
{
    ...
    QG_MPI_VENC_GetH264SpsPpsInfo(mVeChnH264, &pH264SpsPpsInfo);
    ...
    while (iFrameIdx < pVENCParams->iFrameNum)
    {
        ...
        // 获取一帧YUV数据
        VIDEO_FRAME_INFO_S stFrameInfo;
        ret_get = QG_MPI_VI_GetFrame(pVirViParam->iViDev, pVirViParam->iViChn, &stFrameInfo, 500);
        if (ret_get == SUCCESS)
        {                        
 ...
 VIDEO_FORMAT_LINEAR;
            ...
            if(hb_xy->osd_box_num > 0)
            {
                ...
                {
                    ret = QG_MPI_RGN_Create(k, &stRgnAttr);
                    if(ret != SUCCESS)
                    {
                    ...
                    ret = QG_MPI_RGN_AttachToChn(k, &viChn, &stRgnChnAttr);
                    if(ret != SUCCESS)
            ...
            // 送入编码通道
            QG_MPI_VENC_SendFrame(mVeChnH264, &stFrameInfo, 60);
            // 读取编码结果
            int ret = QG_MPI_VENC_GetStream(mVeChnH264, &vencFrame, 20);
            if (SUCCESS == ret)
            {
#ifdef SAVE_H264_FILE
                //保存H264文件
...
#endif
                QG_MPI_VENC_ReleaseStream(mVeChnH264, &vencFrame);
            }
            // 释放YUV数据
            QG_MPI_VI_ReleaseFrame(pVirViParam->iViDev, pVirViParam->iViChn, &stFrameInfo);
        }
    ...
    )
{
		...
		AwG2dInit(test_info.qg_g2d,g2d_param);
		AwG2dProcess(test_info.qg_g2d);
...
//人脸
...
#if TEST_BATCH_G2D
    test_info.qg_g2d = AwG2dCreate();
    if(test_info.qg_g2d == NULL)
    {
        printf("AwG2dCreate failed.\n");
        return -1;
    }
#endif
    test_info.qg_mem = GetMemAdapterOpsS();
    if(test_info.qg_mem == NULL)
    {
    ...
    if(AwMemOpen(test_info.qg_mem,mem_parm) != 0)
    {
   ...
   //alloc 6M continuous phy address for nna
    ...
    if(AwMemPalloc(test_info.qg_mem,nna_buf,1) != 0)
    {
...
#if TEST_BATCH_G2D
    nna_setup_resize_handler(nna_hw_resize_qg);//设置缩放功能回调函数
#else
...
NNA_EXIT:
        ...
        if(test_info.qg_mem != NULL)
        {
            AwMemPfree(test_info.qg_mem, nna_buf, 1);
            AwMemClose(test_info.qg_mem);
        }
#if TEST_BATCH_G2D
        if(test_info.qg_g2d != NULL)
            AwG2dDestroy(test_info.qg_g2d);
#endif
