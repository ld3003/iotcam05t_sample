    ...
    )
{
    ...
    dst_addr_vir = (unsigned char*)AwMemGetVirtualAddressCpu(test_info.qg_mem,(void *)dst_addr);
    ...
    src_addr_vir = (unsigned char*)AwMemGetVirtualAddressCpu(test_info.qg_mem,(void *)src_addr);
    ...
    if(AwG2dProcess(test_info.qg_g2d) == 0){
        /*
...
int main(int argc, char *argv[])
{
   ...
   //alloc 2M for input image
    ...
    if(AwMemPalloc(test_info.qg_mem,nna_buf,2) != 0)
    {
...
#if TEST_BATCH_G2D
    nna_setup_resize_handler(nna_hw_resize_qg);
#else
	...
	if(file != NULL){
			    while(feof(file) == 0)
			    {
			        ...
			        AwMemCopy(test_info.qg_mem,(void *)IMAGE_DDR_VIRADDR,(void *)decode_img,Width*Height);
...
NNA_EXIT:
        ...
        if(test_info.qg_mem != NULL)
        {
            AwMemPfree(test_info.qg_mem, nna_buf, 2);
