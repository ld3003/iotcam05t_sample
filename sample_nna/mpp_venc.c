...
static ERRORTYPE MPPCallbackWrapper(void *cookie, MPP_CHN_S *pChn, MPP_EVENT_TYPE event, void *pEventData)
{
    ...
    case MPP_EVENT_RELEASE_VIDEO_BUFFER:
        if (pFrame != NULL)
        {
            //QG_MPI_VI_ReleaseFrame(0, 0, pFrame);
...
int create_venc(VENC_Params* pVENCParams, VENC_CHN chn, int type)
{
    ...
    if (pVENCParams->srcPixFmt == MM_PIXEL_FORMAT_YUV_QG_AFBC)
    {// 全志私有格式
        mVencChnAttr.VeAttr.PixelFormat = MM_PIXEL_FORMAT_YUV_QG_AFBC;
    ...
    // 创建编码通道
    VENC_CHN mVeChn = chn;
    QG_MPI_VENC_CreateChn(mVeChn, &mVencChnAttr);

    // 设置FrameRate
    VENC_FRAME_RATE_S stFrameRate;
    stFrameRate.SrcFrmRate = stFrameRate.DstFrmRate = pVENCParams->mVideoFrameRate;
    QG_MPI_VENC_SetFrameRate(mVeChn, &stFrameRate);

    // 设置回调函数
    MPPCallbackInfo cbInfo;
    cbInfo.cookie   = pVENCParams;
    cbInfo.callback = (MPPCallbackFuncType)&MPPCallbackWrapper;
    QG_MPI_VENC_RegisterCallback(mVeChn, &cbInfo);
   
    // 启动编码通道
    QG_MPI_VENC_StartRecvPic(mVeChn);
...
{
     // 关闭编码通道
    QG_MPI_VENC_StopRecvPic(chn);

    // 重置编码通道
    QG_MPI_VENC_ResetChn(chn);

    // 销毁编码通道
    QG_MPI_VENC_DestroyChn(chn);
    chn = MM_INVALID_CHN;
...
int save_jpeg_venc(VENC_CHN mVeChn, VENC_CROP_CFG_S* pCrop_cfg, VIDEO_FRAME_INFO_S *pVideoFrame, char* szPicPath)
{
    ...
    // 重置编码通道
    QG_MPI_VENC_StopRecvPic(mVeChn);
    QG_MPI_VENC_ResetChn(mVeChn);
    QG_MPI_VENC_StartRecvPic(mVeChn);
    
    // 更新编码通道参数
    VENC_CHN_ATTR_S mVencChnAttr;
    QG_MPI_VENC_GetChnAttr(mVeChn, &mVencChnAttr);
    mVencChnAttr.VeAttr.AttrJpeg.PicWidth     = pCrop_cfg->Rect.Width;
    mVencChnAttr.VeAttr.AttrJpeg.PicHeight    = pCrop_cfg->Rect.Height;
    QG_MPI_VENC_SetChnAttr(mVeChn, &mVencChnAttr);
    QG_MPI_VENC_SetCrop(mVeChn, pCrop_cfg);
    
	// 送入编码通道
	QG_MPI_VENC_SendFrame(mVeChn, pVideoFrame, 0);
	...
	int result = QG_MPI_VENC_GetStream(mVeChn, &vencFrame, 100);
	if (SUCCESS == result)
	{
		...
		fclose(fd_out);
		QG_MPI_VENC_ReleaseStream(mVeChn, &vencFrame);
	}
	return 0;
}