...
#define ION_IOC_SEAN_POOL_INFO 10
...
typedef struct BUFFER_NODE {
    struct qg_mem_list_head i_list;
...
typedef struct ION_ALLOC_CONTEXT {
    int fd;
    int tarzan_fd;
    struct qg_mem_list_head list; /* buffer list */
...
/*funciton begin*/
int sean_ion_alloc_open(MemParameter param)
{
...
#if 1
    if(param.iommuFlag){
        g_ion_alloc_context->tarzan_fd = open(CEDAR_DEV_NAME, O_RDONLY, 0);
        if (g_ion_alloc_context->tarzan_fd <= 0) {
	        loge("open %s failed\n", CEDAR_DEV_NAME);
		    goto ERROR_OUT;
        }
    }
...
#endif
    QG_MEM_INIT_LIST_HEAD(&g_ion_alloc_context->list);
...
ERROR_OUT:
    ...
    if(param.iommuFlag){
        if (g_ion_alloc_context != NULL
            && g_ion_alloc_context->tarzan_fd > 0) {
            close(g_ion_alloc_context->tarzan_fd);
            g_ion_alloc_context->tarzan_fd = 0;
        }
    }
#endif
...
void sean_ion_alloc_close()
{
    struct qg_mem_list_head * pos, *q;
    ...
    if (--g_ion_alloc_context->ref_cnt <= 0) {
        ...
        qg_mem_list_for_each_safe(pos, q, &g_ion_alloc_context->list) {
            buffer_node * tmp;
            tmp = qg_mem_list_entry(pos, buffer_node, i_list);
...
int sean_ion_alloc_palloc(MemBufInfo* bufferInfo,int /*bufferNum*/ number)
{
...
{
    qg_ion_allocation_info_t alloc_data;
    ion_fd_data_t fd_data;
    struct ion_handle_data handle_data;
    struct qg_ion_custom_info custom_data;
    sean_phys_data   phys_data;
    ...
    alloc_data.qg_len = (size_t)bufferSize;
    alloc_data.qg_align = ION_ALLOC_ALIGN ;

#if 1
    if(g_ion_alloc_context->memParam.iommuFlag)
        //if use iommu,we just don't alloc cache bufer now.
        alloc_data.qg_heap_id_mask = QG_ION_SYSTEM_HEAP_MASK | QG_ION_CARVEOUT_HEAP_MASK;
    else{
        alloc_data.qg_heap_id_mask = QG_ION_DMA_HEAP_MASK | QG_ION_CARVEOUT_HEAP_MASK;
        alloc_data.flags = QG_ION_CACHED_FLAG | QG_ION_CACHED_NEEDS_SYNC_FLAG;
	}
#endif

    ret = ioctl(g_ion_alloc_context->fd, QG_MEM_ION_IOC_ALLOC, &alloc_data);
    if (ret) {
    ...
    ret = ioctl(g_ion_alloc_context->fd, QG_MEM_ION_IOC_MAP, &fd_data);
    if (ret) {
        loge("ION_IOC_MAP err, ret %d, dmabuf fd 0x%08x\n",
				ret, (unsigned int)fd_data.qg_fd);
        goto ALLOC_OUT;
    }

    /* mmap to user */
    addr_vir = (unsigned long)mmap(NULL, alloc_data.qg_len, \
        PROT_READ|PROT_WRITE, MAP_SHARED, fd_data.qg_fd, 0);
    ...
    if(g_ion_alloc_context->memParam.iommuFlag)
    {
        ...
        iommu_param.fd = fd_data.qg_fd;
        ret = ioctl(g_ion_alloc_context->tarzan_fd, QG_MEM_ENGINE_REQ, 0);
        if (ret) {
        ...
        ret = ioctl(g_ion_alloc_context->tarzan_fd, QG_MEM_GET_IOMMU_ADDR, &iommu_param);
        if (ret) {
    ...
    else
    {
        ...
        custom_data.qg_cmd = ION_IOC_SEAN_PHYS_ADDR;
        custom_data.qg_arg = (unsigned long)&phys_data;
        ret = ioctl(g_ion_alloc_context->fd, QG_MEM_ION_IOC_CUSTOM, &custom_data);
        if (ret) {
    ...
    if (alloc_buffer == NULL) {
        ...
        /* unmmap */
        ret = munmap((void*)addr_vir, alloc_data.qg_len);
        if (ret) {
        ...
        ret = ioctl(g_ion_alloc_context->fd, QG_MEM_ION_IOC_FREE, &handle_data);
        if(ret)
    ...
    alloc_buffer->fd_data.qg_fd = fd_data.qg_fd;

    mbufferInfo->dataFd  = alloc_buffer->fd_data.qg_fd;
    ...
    qg_mem_list_add_tail(&alloc_buffer->i_list, &g_ion_alloc_context->list);
...
void sean_ion_alloc_pfree(MemBufInfo* bufferInfo,int number)
{
    int ret,index;
...
for(index = 0;index < number;index++)
{
    ...
    qg_mem_list_for_each_entry(tmp, &g_ion_alloc_context->list, i_list)
    {
        if (tmp->vir == addr_vir) {
       ...
       if(g_ion_alloc_context->memParam.iommuFlag)
       {
	       ret = ioctl(g_ion_alloc_context->tarzan_fd, QG_MEM_ENGINE_REL, 0);
	       if (ret)
              logv("ENGINE_REL failed\n");

	       struct user_iommu_param iommu_param;
           memset(&iommu_param, 0, sizeof(iommu_param));
           iommu_param.fd = tmp->fd_data.qg_fd;
	       ret = ioctl(g_ion_alloc_context->tarzan_fd, QG_MEM_FREE_IOMMU_ADDR, &iommu_param);
           if (ret)
...
void* sean_ion_alloc_vir2phy_cpu(void * pbuf)
{
    int flag = 0;
...
void* sean_ion_alloc_phy2vir_cpu(void * pbuf)
{
    int flag = 0;
    unsigned long addr_vir = 0;
...
void sean_ion_alloc_flush_cache(void* startAddr, int size)
{
    sean_cache_range range;
    int ret;
    ...
    ret = ioctl(g_ion_alloc_context->fd, ION_IOC_SEAN_FLUSH_RANGE, &range);
    if (ret)
        loge("ION_IOC_SEAN_FLUSH_RANGE failed\n");
...
void sean_ion_alloc_flush_cache(void* startAddr, int size)
{
    ...
    custom_data.qg_cmd = ION_IOC_SEAN_FLUSH_RANGE;
    custom_data.qg_arg = (unsigned long)&range;
...
void sean_ion_flush_cache_all()
{
    UNUSA_PARAM(sean_ion_flush_cache_all);

    ioctl(g_ion_alloc_context->fd, ION_IOC_SEAN_FLUSH_ALL, 0);
}

#if 0
void* sean_ion_alloc_alloc_drm(int size)
{
    qg_ion_allocation_info_t alloc_data;
    ion_fd_data_t fd_data;
    struct ion_handle_data handle_data;
    struct qg_ion_custom_info custom_data;
    sean_phys_data   phys_data, tee_data;
    ...
    /*alloc buffer*/
    alloc_data.qg_len = size;
    alloc_data.qg_align = ION_ALLOC_ALIGN ;
    alloc_data.qg_heap_id_mask = QG_ION_SECURE_HEAP_MASK;
    alloc_data.flags = QG_ION_CACHED_FLAG | QG_ION_CACHED_NEEDS_SYNC_FLAG;
    ret = ioctl(g_ion_alloc_context->fd, QG_MEM_ION_IOC_ALLOC, &alloc_data);
    if (ret) {
    ...
    tee_data.size = size;
    custom_data.qg_cmd = ION_IOC_SEAN_TEE_ADDR;
    custom_data.qg_arg = (unsigned long)&tee_data;
    ret = ioctl(g_ion_alloc_context->fd, QG_MEM_ION_IOC_CUSTOM, &custom_data);
    if (ret) {
        loge("ION_IOC_CUSTOM err, ret %d\n", ret);
        addr_phy = 0;
...
int sean_ion_alloc_get_total_size()
{
    int ret = 0;
    int ion_fd = open(DEV_NAME, O_WRONLY);
	struct qg_ion_custom_info cdata;
    ...
    cdata.qg_cmd = ION_IOC_SEAN_POOL_INFO;
    cdata.qg_arg = (unsigned long)&binfo;
    ret = ioctl(ion_fd,QG_MEM_ION_IOC_CUSTOM, &cdata);
    if (ret < 0) {
...
int sean_ion_alloc_memset(void* buf, int value, size_t n)
{
    memset(buf, value, n);
    return -1;
}

int sean_ion_alloc_copy(void* dst, void* src, size_t n)
{
    memcpy(dst, src, n);
    return -1;
}

int sean_ion_alloc_read(void* dst, void* src, size_t n)
{
    memcpy(dst, src, n);
    return -1;
}

int sean_ion_alloc_write(void* dst, void* src, size_t n)
{
    memcpy(dst, src, n);
    return -1;
}

int sean_ion_alloc_setup()
{
    return -1;
}

int sean_ion_alloc_shutdown()
{
    return -1;
}
...
{
    open: 				sean_ion_alloc_open,
    close: 				sean_ion_alloc_close,
    total_size: 		sean_ion_alloc_get_total_size,
    palloc: 			sean_ion_alloc_palloc,
    pfree: 				sean_ion_alloc_pfree,
    flush_cache: 		sean_ion_alloc_flush_cache,
    cpu_get_phyaddr: 	sean_ion_alloc_vir2phy_cpu,
    cpu_get_viraddr: 	sean_ion_alloc_phy2vir_cpu,
    mem_set: 			sean_ion_alloc_memset,
    mem_cpy: 			sean_ion_alloc_copy,
    mem_read: 			sean_ion_alloc_read,
    mem_write: 			sean_ion_alloc_write,
    setup: 				sean_ion_alloc_setup,
    shutdown: 			sean_ion_alloc_shutdown
};
