...
/*
    ...
    QG_MPI_VI_GetFrame 获取视频帧
    QG_MPI_VI_ReleaseFrame 释放视频帧
*/
int create_vi(VirVi_Params* pVirViParams)
{
    ...
    {   
        ...
        {// Create VIPP
            ...
            QG_MPI_VI_CreateVipp(VIPPIndex); //创建VIPP物理设备
            QG_MPI_VI_SetVippAttr(VIPPIndex, &stVIPP_Attr);//设置VIPP物理设备属性
            QG_MPI_VI_SetVippFlip(VIPPIndex, g_VIPP_map[VIPPIndex].bFlip);
            QG_MPI_VI_SetVippMirror(VIPPIndex, g_VIPP_map[VIPPIndex].bMirror);
            QG_MPI_VI_EnableVipp(VIPPIndex);//启动VIPP物理设备
        ...
        // 启动VirVi通道
        QG_MPI_VI_CreateVirChn(pVirViParams[VirViIndex].iViDev, pVirViParams[VirViIndex].iViChn, NULL);
        //基于某个VIPP，创建虚通道
        QG_MPI_VI_SetVirChnAttr(pVirViParams[VirViIndex].iViDev, pVirViParams[VirViIndex].iViChn, NULL);
        //设置虚通道属性
        QG_MPI_VI_EnableVirChn(pVirViParams[VirViIndex].iViDev, pVirViParams[VirViIndex].iViChn);
        //启动虚通道
        printf("\n--[W:%d H:%d]--\n",g_VIPP_map[VIPPIndex].width,g_VIPP_map[VIPPIndex].height);
    }
    // Start isp server
    QG_MPI_ISP_Init();//初始化 ISP 设备
    QG_MPI_ISP_Run(0);//运行ISP设备
...
int destroy_vi(VirVi_Params* pVirViParams)
{
    ...
    for (int VirViIndex = 0; VirViIndex < VirVi_MAX; VirViIndex++)
    {
        ...
        // 销毁VirVi通道
        QG_MPI_VI_DisableVirChn(pVirViParams[VirViIndex].iViDev, pVirViParams[VirViIndex].iViChn);
        QG_MPI_VI_DestoryVirChn(pVirViParams[VirViIndex].iViDev, pVirViParams[VirViIndex].iViChn);
        
        if (g_VIPP_map[VIPPIndex].bInit == 1)
        {
            QG_MPI_VI_DisableVipp(VIPPIndex);
            QG_MPI_VI_DestoryVipp(VIPPIndex);
