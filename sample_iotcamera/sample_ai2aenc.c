/******************************************************************************
  Copyright (C), 2001-2016, Qigan Tech. Co., Ltd.
 ******************************************************************************
  File Name     :
...
static ERRORTYPE createAiChn(SampleAi2AencContext *pStContext)
{
    ...
    QG_MPI_AI_SetPubAttr(pStContext->mAiDev, &pStContext->mAiAttr);
    QG_MPI_AI_Enable(pStContext->mAiDev);

    pStContext->mAiChn = 0;
    while (pStContext->mAiChn < AIO_MAX_CHN_NUM)
    {
        ret = QG_MPI_AI_CreateChn(pStContext->mAiDev, pStContext->mAiChn);
        if (SUCCESS == ret)
        {
            bSuccessFlag = TRUE;
...
int aenc_main(int argc, char **argv)
{
    ...
    QG_MPI_AENC_StopRecvPcm(stContext.mAencChn);
...
_exit_2:
    ...
    {
        QG_MPI_AI_ResetChn(stContext.mAiDev, stContext.mAiChn);
        QG_MPI_AI_DestroyChn(stContext.mAiDev, stContext.mAiChn);
        QG_MPI_AI_Disable(stContext.mAiDev);
    }
    if (stContext.mAencChn >= 0)
    {
        QG_MPI_AENC_ResetChn(stContext.mAencChn);
        QG_MPI_AENC_DestroyChn(stContext.mAencChn);
    }
_exit_1:
    QG_MPI_SYS_Exit();
