...
int msgarrvd(void *context, char *topicName, int topicLen, MQTTClient_message *message)
{
	...
	printf("topic: %s\n", topicName);
#if 1
	char msg[MQTT_MAX_MSGLEN]={0};
	memcpy(msg,message->payload,message->payloadlen);
	if (strstr(topicName, mqttclientid))
	{
		if (p2psignal_cb)
		{
			p2psignal_cb(0, (char *)msg);
		}
	}
#endif
...
CONNECT:
	...
	//sub
	snprintf(topicbuf, sizeof(topicbuf), "general/3/%s/down", mqttclientid);
...
int pubmsg(char *msg)
{
	...
	if (mqttcd.conn == 1)
	{
		...
		pubmsg.qos = QOS;
		pubmsg.retained = 0;
		snprintf(topicbuf, sizeof(topicbuf), "general/3/%s/up", mqttclientid);
