...
int Read_One_Aac_Frame(TSPKG_SESSION *session,unsigned char * buf)
{
...
int AAC2PES(TSPKG_SESSION *session,TsPes * tsaacpes,unsigned long Adudiopts)
{
	unsigned int aacpes_pos = 0;
	unsigned int OneFrameLen_AAC = 0;

	//读取出一帧数据
	OneFrameLen_AAC = Read_One_Aac_Frame(session,tsaacpes->Es);
...
int AAC2PES(TSPKG_SESSION *session, TsPes * tsaacpes, unsigned long Adudiopts, unsigned char *pData, int iDataSize)
{
	unsigned int aacpes_pos = 0;
	unsigned int OneFrameLen_AAC = 0;
