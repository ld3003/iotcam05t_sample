...
int GetFrameType(NALU_t *nal)
{
	...
	{
		printf("Error malloc OneFrameBuf_H264 [%d]\n", nal->len + 4);
		return getchar();
	}
...
int H2642PES(TSPKG_SESSION *session, TsPes *tsh264pes, unsigned long Videopts, unsigned int *videoframetype)
{
	...
	//读取出一帧数据
	OneFrameLen_H264 = Read_One_H264_Frame(session, tsh264pes->Es, videoframetype);
	//从流获取数据

	h264pes_pos += OneFrameLen_H264;

	//printf("h264pes_pos %08x OneFrameLen_H264 %08x \n",h264pes_pos,OneFrameLen_H264);
...
int H2642PES(TSPKG_SESSION *session, TsPes *tsh264pes, unsigned long Videopts, unsigned int *videoframetype, unsigned char *pData, int iDataSize)
{
	...
	memset(tsh264pes->Es, 0, MAX_ONE_FRAME_SIZE);
	//读取出一帧数据
	OneFrameLen_H264 = Read_One_H264_Frame(session, tsh264pes->Es, videoframetype, pData, iDataSize);
	//从流获取数据

	h264pes_pos += OneFrameLen_H264;
	//printf("h264pes_pos %d OneFrameLen_H264 %d \n",h264pes_pos,OneFrameLen_H264);
