...
int WriteStruct_Packetheader(TSPKG_SESSION *session, unsigned char *Buf, unsigned int PID, unsigned char play_init, unsigned char ada_field_C)
{
	...
	{
		tspacketheader.conti_cter = (session->continuity_counter.continuity_counter_pat % 16);
		session->continuity_counter.continuity_counter_pat++;
	}
	else if (PID == TS_PMT_PID) //这是pmt的包
	{
		tspacketheader.conti_cter = (session->continuity_counter.continuity_counter_pmt % 16);
		session->continuity_counter.continuity_counter_pmt++;
	}
	else if (PID == TS_H264_PID) //这是H264的包
	{
		tspacketheader.conti_cter = (session->continuity_counter.continuity_counter_video % 16);
		session->continuity_counter.continuity_counter_video++;
	}
	else if (PID == TS_AAC_PID) //这是MP3的包
	{
		tspacketheader.conti_cter = (session->continuity_counter.continuity_counter_audio % 16);
		session->continuity_counter.continuity_counter_audio++;
	}
	else //其他包出错，或可扩展
	{
...
int WriteStruct_Pat(TSPKG_SESSION *session, unsigned char *Buf)
{
	...
	pat_pos += WriteStruct_Packetheader(session, Buf, TS_PAT_PID, 0x01, 0x01); //PID = 0x00,有效荷载单元起始指示符_play_init = 0x01, ada_field_C,0x01,仅有有效负载 ；
	Buf[4] = 0;																   //自适应段的长度为0
	pat_pos++;
...
int WriteStruct_Pmt(TSPKG_SESSION *session, unsigned char *Buf)
{
	...
	pmt_pos += WriteStruct_Packetheader(session, Buf, TS_PMT_PID, 0x01, 0x01); //PID = TS_PMT_PID,有效荷载单元起始指示符_play_init = 0x01, ada_field_C,0x01,仅有有效负载；
